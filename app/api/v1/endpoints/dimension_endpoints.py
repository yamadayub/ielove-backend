...
async def update_product_dimensions(
    product_id: int,
    dimensions_data: schemas.ProductDimensionsUpdateSchema,
    db: Session = Depends(get_db),
    current_user: schemas.UserSchema = Depends(get_current_user)
):
...
@router.patch("/dimensions/{dimension_id}", response_model=schemas.ProductDimensionSchema, summary="製品寸法を更新する")
