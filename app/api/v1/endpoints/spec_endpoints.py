...
async def update_product_specifications(
    product_id: int,
    specs_data: schemas.ProductSpecificationsUpdateSchema,
    db: Session = Depends(get_db),
    current_user: schemas.UserSchema = Depends(get_current_user)
):
...
@router.patch("/specifications/{spec_id}", response_model=schemas.ProductSpecificationSchema, summary="製品仕様を更新する")
